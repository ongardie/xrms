...
if ($rst) {
    while (!$rst->EOF) {
        ...
        $contact_rows .= '<td class="widget_content">' . $rst->fields['email'] . '</td>';
                                $contact_rows .= '<td class="widget_content"><a href="email_merge_preview.php?contact_id='.$rst->fields['contact_id'].'" target=_blank>Preview</a></td>';
        $contact_rows .= "</tr>\n";
        $rst->movenext();
    }
            ...
            <tr>
              <td class="widget_content_form_element" colspan="6">
                    <input type="submit" class="button" value="<?php echo _("Continue"); ?>">&nbsp;
                                        ...
                                        <input type=button value="Switch All" onClick="switchAll('sendmail','array_of_contacts_',<?php echo $count; ?>)"> 
                                        <?php if ($show_opt_out) {?>
                                        <input type="checkbox" name="generic_message" value=true checked>Save database space - use generic description?&nbsp;
                                        <?php } ?><br>
 ...
 *
 * Revision 1.20z 2007/02/04 randym
 * - added ability to check, uncheck or switch all selected records
 *
