...
*
* $Id: email-4.php,v 1.1 2008/03/15 16:54:31 randym56 Exp $
*
...
if ($rst) {
        ...
        */
        ...
        while (!$rst->EOF)
        {
				...
				$m=mail_merge_email($con,$email_template_title,$output1,$rst->fields['contact_id'],false);
				$msg_subject=$m[0];
				$msg_body=$m[1];

//echo stripslashes($msg_body); //debug
				$objSMTP = new SMTPs ();
				$objSMTP->setConfig($con);
				...
				$objSMTP->setSensitivity(0); //1 = personal
                        ...
                        if ($m) $mail_result=$objSMTP->sendMsg ('smtp.att.yahoo.com','jimrandy@sbcglobal.net','');
                        ...
                        if ($_POST['generic_message']) $activity_data['activity_description'] = 'Bulk email sent';
							else $activity_data['activity_description'] = addslashes($txt);
                        $activity_data['activity_status']      = 'c';         // Closed status
                        $activity_data['completed_bol']        = true;           // activity is completed
						$activity_data['workflow'] 			   = 0;
                        ...
                        if ( $activity_id = add_activity($con, $activity_data, 0, $participants ) )
                        {
...
//send only one copy of a bulk mail to the BCC to prevent multiple BCCs
        ...
        //$objSMTP->setConfig2($con);  // was set earlier - cannot set twice
        //$objSMTP->setFrom ( $sender_name . '<' . $sender_address . '>' );
        ...
        //this line of code sends the message to the SMTP server
        $mail_result=$objSMTP->sendMsg ('smtp.att.yahoo.com','jimrandy@sbcglobal.net','');
        }
