...
// ** Basic System configuration
    ...
    *
    */
    function setConfig ($con, $user_id=null)
    {
       /**
        * Default return value
        *
        ...
        */
		...
		$this->_smtpsPort   = null; 
		$this->_msgReplyTo  = null;
		$this->_smtpsHost   = null;
		$this->_smtpsID     = null;
		$this->_smtpsPW     = null;

		@require_once ('SMTPs-get-auth.php');
		$username = set_SMTPs_defaults($con, $user_id, &$smtpsPort, &$msgReplyTo, &$smtpsHost, &$smtpsID, &$smtpsPW);
		$_retVal = true;

//	echo $msgReplyTo."<br>".$smtpsID."<br>".$smtpsPW."<br>".$smtpsHost."<br>".$smtpsPort; exit;

		if (!$msgReplyTo || !$smtpsID || !$smtpsPW || !$smtpsHost || !$smtpsPort) {
            // Otherwise return error.
            
                $this->_setErr ( 110, 'Invalid SMTP settings in User record: ('.$username.') - contact system administrator.' );
                $_retVal = false;
            
        	}
		else
			{	
			$this->_smtpsPort 	= $smtpsPort; 
			$this->_msgReplyTo 	= $msgReplyTo;
			$this->_smtpsHost 	= $smtpsHost;
			$this->_smtpsID 	= $smtpsID;
			$this->_smtpsPW 	= $smtpsPW;
