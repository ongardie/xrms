...
class iCal {
	...
	*/
	var $prodid = '-//amgcrm.com//iCal Class MIMEDIR//EN';
		...
		if (is_object($alarm)) {
			if ($format === 'ics') {
				if ($alarm->getTrigger() > 0) {
					$output .= (string) "X-MICROSOFT-CDO-BUSYSTATUS:BUSY\r\n";
					$output .= (string) "X-MICROSOFT-CDO-IMPORTANCE:1\r\n";
					...
					if (($alarm->getAction() == 'DISPLAY' || $alarm->getAction() == 'EMAIL') AND ($alarm->getSummary() > '')) {
	...
	*/
	function outputFile($format = 'ics') {
		if ($format == 'ics') {
			...
			header('Content-Disposition: attachment; filename=iCalendar_' . date('Y-m-d_H-m-s') . '.ics');
			echo $this->getOutput('ics');
		} elseif ($format == 'xcs') {
			header('Content-Type: text/Calendar');
			header('Content-Disposition: attachment; filename=iCalendar_' . date('Y-m-d_H-m-s') . '.xcs');
			echo $this->getOutput('xcs');
		} elseif ($format == 'rdf') {
			header('Content-Type: text/xml');
			header('Content-Disposition: attachment; filename=iCalendar_' . date('Y-m-d_H-m-s') . '.rdf');
