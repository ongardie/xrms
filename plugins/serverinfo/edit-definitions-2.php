<?php
/**
 * Edit serverinfo element definitions
 *
 * $Id: edit-definitions-2.php,v 1.1 2004/07/06 19:57:02 gpowers Exp $
 */
...
$con2 = &adonewconnection($xrms_db_dbtype);
$con2->connect($xrms_db_server, $xrms_db_username, $xrms_db_password, $xrms_db_dbname);
//$con2->debug = 1;
...
$sql = "SELECT * FROM svrinfo_element_definitions WHERE element_enabled=1";
...
while (!$all_elements->EOF) {
  ...
  $do_update = false;
  $sql2 = "UPDATE svrinfo_element_definitions SET ";
  ...
  if ($do_update) {
    ...
    $status = $con2->execute($sql2);
    if (!$status) {
      db_error_handler ($con2, $sql2);
      exit;
    }
...
if ($new_element) {
  ...
  #echo "<pre>";var_dump($fields);echo "<br>";var_dump($values);echo "</pre>";exit;
  $sql = "INSERT INTO svrinfo_element_definitions (";
  $first_time = true;
...
$con->close();
$con2->close();
$url = "$http_site_root/plugins/serverinfo/edit.php?company_id=$company_id&server_id=$server_id&return_url=$return_url";
header("Location: $url");
