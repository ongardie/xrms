...
$sql .= "WHERE svrinfo_element_definitions.element_enabled=1 ";
...
$sql2 = "SELECT value, element_id FROM svrinfo WHERE server_id='$server_id'";
...
if ($all_elements) {
    while (!$all_elements->EOF) {
        ...
        $contact_rows .= "<td class=widget_content><a href='../contacts/one.php?contact_id="
                        . $all_elements->fields['contact_id'] . "'>"
                        . $all_elements->fields['last_name'] . ', ' . $all_elements->fields['first_names']
                        . '</a></td>';
        $contact_rows .= '<td class=widget_content>' . $all_elements->fields['summary'] . '</td>';
        $contact_rows .= '<td class=widget_content>' . $all_elements->fields['title'] . '</td>';
        $contact_rows .= '<td class=widget_content>' . $all_elements->fields['description'] . '</td>';
        $contact_rows .= '<td class=widget_content>' . $all_elements->fields['work_phone'] . '</td>';
        $contact_rows .= "\n\t<td class=widget_content><a href='mailto:"
                        . $all_elements->fields['email']
...
if ($all_elements) {
    while (!$all_elements->EOF) {
        ...
        $former_name_rows .= '<td class=clear>' . $all_elements->fields['former_name'] . '</td>';
        $former_name_rows .= '</tr>';
        $all_elements->movenext();
    }
    $all_elements->close();
...
if ($all_elements) {
    while (!$all_elements->EOF) {
        $linecounter +=1;
        $established_at = $con->userdate($all_elements->fields['established_at']);
        $relationship_rows .= ($linecounter == '1') ? '<tr><td class=sublabel>Relationship</td>' : '<tr><td class=sublabel>&nbsp;</td>';
        $relationship_rows .= '<td class=clear>' . $all_elements->fields['relationship_type'] . ' '
            . '<a href="one.php?company_id=' . $all_elements->fields['company_to_id']
            . '">' . $all_elements->fields['company_name'] . '</a> '
            . $established_at . '</td>';
