	...
	*/
	function create( &$xmls ) {
		...
		if( isset( $this->data ) ) {
                        ...
                        $datasql=$this->data->create( $xmls );

                        if ($xmls->seperateDataSQL) {
                            $xmls->addDataSQL($datasql);
                        } else {
                            $sql[] = $datasql;
                        }
		}
	...
	*/
	function create( &$xmls ) {
		...
		foreach( $this->data as $row ) {
			...
			foreach( $row as $field_id => $field_data ) {
				if( !array_key_exists( $field_id, $table_fields ) ) {
					if( is_numeric( $field_id ) ) {
                        $field_keys= array_keys( $table_fields );
						$field_id = reset( $field_keys );
					} else {
						continue;
					}
				}
			...
			$field_headers=array_keys($fields);
                        foreach ($field_headers as $hkey=>$hval) {
                            $field_headers[$hkey]=$xmls->dict->NameQuote($hval);
                        }
			...
			if( !in_array( $table, $tables ) or ( $mode = $xmls->existingData() ) == XMLS_MODE_INSERT ) {
				...
				$sql[] = 'INSERT INTO '. $table .' ('. implode( ',', $field_headers ) .') VALUES ('. implode( ',', $fields ) .')';
				continue;
			}
...
class adoSchema {
	...
	*/
	...
	/**
	* @var array	Array containing SQL queries to generate all data entries
	* @access private
	*/
	var $sqlDataArray;
	...
	*/
	...
	/**
	* @var boolean Option to write Data SQL seperately from table SQL
	*/
        var $seperateDataSQL;
	...
	*/
	...
	/**
	* Adds data SQL into a seperate SQL array.
	*
	...
	*/
	function getSQL( $format = NULL, $sqlArray = NULL ) {
		if( !is_array( $sqlArray ) ) {
			$sqlArray = $this->sqlArray;
                        if ($this->seperateDataSQL AND is_array($this->sqlDataArray)) {
                            $sqlArray=array_merge($sqlArray,$this->sqlDataArray);
                        }
		}
	...
	/**
	* Gets the Data SQL array in the specified format.
	*
	* @param string $format Format
	* @return mixed SQL
	*	
	* @access private
	*/
	function getDataSQL( $format = NULL, $sqlArray = NULL ) {
            if( !is_array( $sqlArray ) ) {
			$sqlArray = $this->sqlDataArray;
		}
                return $this->getSQL($format, $sqlArray);
	}
