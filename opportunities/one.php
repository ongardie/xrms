...
$sql_activities = "select activity_id, activity_title, scheduled_at, a.entered_at, activity_status, at.activity_type_pretty_name, u.username, if(activity_status = 'o' and scheduled_at < now(), 1, 0) as is_overdue
from activity_types at, users u, activities a
where a.on_what_table = 'opportunities' and on_what_id = $opportunity_id
...
if ($rst) {
    while (!$rst->EOF) {
        ...
        $activity_rows .= '<tr>';
        $activity_rows .= "<td class='$classname'><a href='$http_site_root/activities/one.php?return_url=/opportunities/one.php?opportunity_id=$opportunity_id&activity_id=" . $rst->fields['activity_id'] . "'>" . $rst->fields['activity_title'] . '</a></td>';
