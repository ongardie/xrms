...
for ($i=0; $i < $how_many_rows_to_import_per_page; $i++) {
    ...
    if (strlen($company_name) > 0) {
        ...
        $sql_insert_company = "insert into companies (user_id, crm_status_id, company_source_id, industry_id, account_status_id, rating_id, company_name, address, city, state, postal_code, phone) values ($user_id, $crm_status_id, $company_source_id, $industry_id, $account_status_id, $rating_id, " . $con->qstr($company_name, get_magic_quotes_gpc()) . ", ". $con->qstr($company_address, get_magic_quotes_gpc()) . ", " . $con->qstr($company_city, get_magic_quotes_gpc()) . ", " . $con->qstr($company_state, get_magic_quotes_gpc()) . ", " . $con->qstr($company_postal_code, get_magic_quotes_gpc()) . ", " . $con->qstr($company_phone, get_magic_quotes_gpc()) . ")";
