...
for ($i=0; $i < $how_many_rows_to_import_per_page; $i++) {
    ...
    if (strlen($company_name) > 0) {
        ...
        $sql_insert_address = "insert into addresses (company_id, address_name, line1, city, province, postal_code) values ($company_id, " .$con->qstr($address_body, get_magic_quotes_gpc()) . ", " .$con->qstr($company_address, get_magic_quotes_gpc()) . ", " .$con->qstr($company_city, get_magic_quotes_gpc()) . ", " .$con->qstr($company_state, get_magic_quotes_gpc()) . ", " .$con->qstr($company_postal_code, get_magic_quotes_gpc()) . ");";
