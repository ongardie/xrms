...
$sql = "update activity_templates set activity_template_record_status='d' where activity_template_id=$activity_template_id";
$con->execute($sql);
//$con->debug = 1;
...
for ($i = 1; $i <= $max; $i++) {
    ...
    $sql = "update activity_templates set sort_order=$i where activity_template_id=$activity_template_id";
    $con->execute($sql);
    $rst->movenext();
}
$rst->close();
