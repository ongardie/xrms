 ...
 * 
 * $Id: reports.php,v 1.1 2004/04/20 20:03:18 braverock Exp $ 
 */ 
...
$sql_user = "select username,last_name,first_names,user_id from users
where user_id in ";
...
//		$tab_both = $tab_both . $tab_act . '</td>';
		$tab_opp .= '</td>';
		
		if ($rst_case->NumRows() > 0) {
//			$tab_both .= '<td colspan="4">';
			$tab_opp .= '<td colspan="4">';
		}
		else
		{
//			$tab_both .= '<td colspan="4" class=widget_content>';
...
//				$tab_case .= '</table>';
		...
		$rst_user->movenext();
//		$tab_both = $tab_both . $tab_case .'</td></tr>';
		$tab_opp .= '</td></tr>';
	}
	$rst_user->close();
}
...
//	$tab_empty = '<tr><td colspan="7" class="widget_content"> No Records Found Here </td></tr>';
    ...
    <td class=lcol width=100% valign=top> 
      <table cellspacing=1 width=100%>
        ...
        <tr> 
          ...
          <td class=widget_label>Opportunity (Sort 1 - company name, Sort 2 - 
            Value)</td>
        ...
        <?php  } else { ?>
        <tr> 
          <td height="22" colspan="7" class="widget_content">Invalid Date Entered, 
            Please Enter in YYYY-MM-DD Format.</td>
        </tr>
        <?php  }  ?>
      </table>
    </td>
